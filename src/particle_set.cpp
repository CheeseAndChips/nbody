...
void particle_set_t::simulation_timestep(int32_t j, scalar_t deltaT, scalar_t gravitationalConstant, scalar_t distanceAdded) {
...
void particle_set_t::update_particle_position(int32_t j, scalar_t deltaT)
{
    positions[j].x += velocities[j].x * deltaT;
    positions[j].y += velocities[j].y * deltaT;
}

void particle_set_t::threaded_timestep(int32_t threadnum, int32_t threadcnt, scalar_t deltaT, scalar_t gravitationalConstant, scalar_t distanceAdded)
{
    for(int i = threadnum; i < n; i += threadcnt)
    {
        simulation_timestep(i, deltaT, gravitationalConstant, distanceAdded);
    }
}

void particle_set_t::threaded_position_update(int32_t threadnum, int32_t threadcnt, scalar_t deltaT)
{
    for(int i = threadnum; i < n; i += threadcnt)
    {
        update_particle_position(i, deltaT);
    }
}