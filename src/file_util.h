...
template<typename T>
void writeToFile(std::ostream& file, const T& a)
{
    const char* binaryData = reinterpret_cast<const char*>(&a);
    file.write(binaryData, sizeof(T));
}

void writeToFile(std::ostream& file, const vec2d_t& a)
{
...
template<typename T>
void readFromFile(std::istream& file, T& a)
{
    char* binaryData = reinterpret_cast<char*>(&a);
    file.read(binaryData, sizeof(T));
}

void readFromFile(std::istream& file, vec2d_t& a)
{
